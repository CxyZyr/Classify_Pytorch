...
class Depth_Wise_R(Module):
    def __init__(self, c1, c2, c3, residual=True, kernel=(3, 3), stride=(2, 2), padding=(1, 1), groups=1):
        super(Depth_Wise_R, self).__init__()
        c1_in, c1_out = c1
        c2_in, c2_out = c2
        c3_in, c3_out = c3
    ...
    def forward(self, x):
        ...
        x = self.conv_dw(x)
        x = self.project(x)
        output = self.qat_add.add(short_cut, x)

        return output


class Depth_Wise_N(Module):
    def __init__(self, c1, c2, c3, residual=False, kernel=(3, 3), stride=(2, 2), padding=(1, 1), groups=1):
        super(Depth_Wise_N, self).__init__()
        c1_in, c1_out = c1
        c2_in, c2_out = c2
        c3_in, c3_out = c3
        self.conv = Conv_block(c1_in, out_c=c1_out, kernel=(1, 1), padding=(0, 0), stride=(1, 1))
...
class Residual(Module):
    def __init__(self, c1, c2, c3, num_block, groups, kernel=(3, 3), stride=(1, 1), padding=(1, 1)):
        ...
        for i in range(num_block):
            ...
            c2_tuple = c2[i]
            c3_tuple = c3[i]
            modules.append(Depth_Wise_R(c1_tuple, c2_tuple, c3_tuple, residual=True,
...
class SEModule(Module):
    ...
    def forward(self, x):
        ...
        x = self.bn2(x)
        x = self.sigmoid(x)
        return self.qat_add.mul(module_input,x)
    ...
    def __init__(self, c1, c2, c3, num_block, groups, kernel=(3, 3), stride=(1, 1), padding=(1, 1), se_reduct=4):
        ...
        for i in range(num_block):
            ...
            if i == num_block-1:
                ...
                modules.append(Depth_Wise_R(c1_tuple, c2_tuple, c3_tuple, residual=True, kernel=kernel, padding=padding,
...
class QATMiniFASNet(Module):
    def __init__(self, keep, embedding_size, conv6_kernel=(7, 7),
        ...
        self.conv1 = Conv_block(img_channel, keep[0], kernel=(3, 3), stride=(2, 2), padding=(1, 1))  # 40x40
        ...
        self.conv_23 = Depth_Wise_N(c1[0], c2[0], c3[0], kernel=(3, 3), stride=(2, 2), padding=(1, 1),
                                    groups=keep[3])  # 20x20
        ...
        self.conv_34 = Depth_Wise_N(c1[0], c2[0], c3[0], kernel=(3, 3), stride=(2, 2), padding=(1, 1),
                                    groups=keep[19])  # 10x10
        ...
        self.conv_45 = Depth_Wise_N(c1[0], c2[0], c3[0], kernel=(3, 3), stride=(2, 2), padding=(1, 1),
                                    groups=keep[40])  # 5x5
        ...
        self.merge_linear = Linear(512, embedding_size, bias=True)
        self.drop = torch.nn.Dropout(p=drop_p)
        self.prob = Linear(embedding_size, num_classes, bias=False)

    def apply_to_all_custom_conv(self):
        for module in self.modules():
            if isinstance(module, Conv_block):
                torch.quantization.fuse_modules(module, ['conv', 'bn'], inplace=True)
            if isinstance(module, Linear_block):
                torch.quantization.fuse_modules(module, ['conv', 'bn'], inplace=True)
            if isinstance(module, SEModule):
                torch.quantization.fuse_modules(module, ['fc1', 'bn1'], inplace=True)
                torch.quantization.fuse_modules(module, ['fc2', 'bn2'], inplace=True)
...
class QATMiniFASNetSE(QATMiniFASNet):
    def __init__(self, keep, embedding_size, conv6_kernel=(7, 7),drop_p=0.75, num_classes=4, img_channel=3):
        super(QATMiniFASNetSE, self).__init__(keep=keep, embedding_size=embedding_size, conv6_kernel=conv6_kernel,
...
def QATMiniFASNetV1(embedding_size=128, conv6_kernel=(5, 5),
                 drop_p=0.2, num_classes=3, img_channel=3):
    return QATMiniFASNet(keep_dict['1.8M'], embedding_size, conv6_kernel, drop_p, num_classes, img_channel)

# (80x80) flops: 0.044, params: 0.43
def QATMiniFASNetV2(embedding_size=128, conv6_kernel=(5, 5),
                     drop_p=0.2, num_classes=3, img_channel=3):
    return QATMiniFASNet(keep_dict['1.8M_'], embedding_size, conv6_kernel, drop_p, num_classes, img_channel)

def QATMiniFASNetV1SE(embedding_size=128, conv6_kernel=(5, 5),
                   drop_p=0.75, num_classes=3, img_channel=3):
    return QATMiniFASNetSE(keep_dict['1.8M'], embedding_size, conv6_kernel,drop_p, num_classes, img_channel)

# (80x80) flops: 0.044, params: 0.43
def QATMiniFASNetV2SE(embedding_size=128, conv6_kernel=(5, 5),
                   drop_p=0.75, num_classes=3, img_channel=3):
    return QATMiniFASNetSE(keep_dict['1.8M_'], embedding_size, conv6_kernel,drop_p, num_classes, img_channel)
